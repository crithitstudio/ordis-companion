...
const SAMPLE_WEAPONS: Record<
  string,
  { disposition: number; type: "primary" | "secondary" | "melee" }
> = {
...
export function RivenView() {
  ...
  const [config, setConfig] = useState<RivenConfig>({
    positiveStats: 2,
    hasNegative: false,
  });
  ...
  const [activeTab, setActiveTab] = useState<"calculator" | "inventory">(
    "calculator",
  );
  ...
  const incrementRoll = useCallback(
    (id: string) => {
      updateRivens(
        ownedRivens.map((r) =>
    ...
    (baseStat: number) => {
      const dispMult =
        DISPOSITION_MULTIPLIERS[
          selectedDisposition as keyof typeof DISPOSITION_MULTIPLIERS
        ];
    ...
    <div className="space-y-6">
      ...
      <section className="bg-gradient-to-r from-purple-900/30 to-slate-900/50 rounded-xl border border-purple-700/30 p-6">
        ...
        <p className="text-slate-400 text-sm">
          Calculate stat ranges based on weapon disposition and riven
          configuration.
        </p>
      </section>
        ...
        <>
                  ...
                  {filteredWeapons.map(([name, data]) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-slate-500">
                        {"●".repeat(data.disposition)}
                        {"○".repeat(5 - data.disposition)} • {data.type}
                      </span>
                    </button>
                  ))}
                </div>
              ...
              <div className="mt-3 p-3 bg-purple-900/20 border border-purple-700/30 rounded-lg flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-slate-400">
                    {SAMPLE_WEAPONS[selectedWeapon]?.type}
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="text-xs text-slate-500">
                    Disposition {selectedDisposition}/5
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-4">
            ...
            <div className="text-center p-2 bg-slate-800/50 rounded-lg">
              ...
              <span
                className={`font-bold ${getStatModifier(config) > 1 ? "text-green-400" : "text-yellow-400"}`}
              >
              ...
              {RIVEN_STATS.map((stat) => {
                ...
                return (
                  <button
                    key={stat.name}
                    onClick={() =>
                      setSelectedStat(isSelected ? null : stat.name)
                    }
        ...
        <div className="bg-slate-900/50 rounded-xl border border-purple-700/30 p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1 text-xs text-slate-400">
                ...
                <select
                  value={sortBy}
                  onChange={(e) =>
                    setSortBy(e.target.value as "weapon" | "rolls" | "date")
                  }
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 mb-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setNewRiven({ ...newRiven, weapon: e.target.value })
                  }
                ...
                <input
                  ...
                  value={newRiven.name || ""}
                  onChange={(e) =>
                    setNewRiven({ ...newRiven, name: e.target.value })
                  }
                  className="bg-slate-700 border border-slate-600 rounded px-3 py-2 text-sm"
                />
              </div>
              <input
                ...
                onChange={(e) =>
                  setNewRiven({ ...newRiven, notes: e.target.value })
                }
              ...
              <div className="flex gap-2">
                <button
                  onClick={addRiven}
                  className="flex-1 py-2 bg-purple-600 text-white rounded hover:bg-purple-500 text-sm"
                >
                  Save Riven
                </button>
                <button
                  onClick={() => setShowAddRiven(false)}
          ...
          {ownedRivens.length === 0 ? (
            <p className="text-center text-slate-500 py-8">
              No rivens tracked yet. Add your first riven above!
            </p>
          ) : (
                  ...
                  >
                    <div className="flex-1">
                      <div className="font-medium text-slate-200">
                        {riven.weapon} {riven.name}
                      </div>
                      <div className="text-xs text-slate-500">
                        {riven.notes || "No notes"}
                      </div>
                    </div>
      ...
      <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
        ...
        <ul className="text-sm text-slate-400 space-y-1">
          ...
          <li>
            • Kuva rerolls cost increases: 900 → 1400 → 1900... (caps at 3500)
          </li>
          <li>
            • Use <strong>Semlar</strong> or <strong>riven.market</strong> for
            pricing
          </li>
        </ul>
      </div>

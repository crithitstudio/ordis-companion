...
import { Users, Star, Plus, Minus, RefreshCw, ExternalLink } from "lucide-react";
...
const SYNDICATES = [
    {
        id: "steel-meridian",
        name: "Steel Meridian",
        description: "The Steel Meridian are made up of Grineer defectors and soldiers who have turned their back on the empire.",
        color: "text-red-400",
    ...
    },
    {
        ...
        description: "The Arbiters of Hexis seek to uncover the truth behind Warframe and their Tenno operators.",
        color: "text-blue-400",
    ...
    },
    {
        id: "cephalon-suda",
        name: "Cephalon Suda",
        description: "Cephalon Suda has devoted herself to the pursuit of knowledge and cataloging the origin system.",
        color: "text-cyan-400",
    ...
    },
    {
        ...
        description: "The Perrin Sequence reject the Corpus ideology of profit through conflict.",
    ...
    },
    {
        id: "red-veil",
        name: "Red Veil",
        description: "Red Veil sees corruption all around them. They are honor-bound to root it out.",
        color: "text-rose-400",
    ...
    },
    {
        id: "new-loka",
        name: "New Loka",
        description: "New Loka believes in humanity's purity, and seeks to restore Earth to its former glory.",
        color: "text-lime-400",
...
const OPEN_WORLD_SYNDICATES = [
    {
        ...
        description: "The Ostrons of Cetus, dwellers of the ancient Orokin Tower Unum.",
    ...
    },
    {
        ...
        description: "Workers of Fortuna who resist the Corpus rule over Orb Vallis.",
        color: "text-blue-400",
        borderColor: "border-blue-700/50",
    ...
    },
    {
        ...
        description: "The ancient Orokin family who studies the Infestation on Deimos.",
    ...
    },
    {
        ...
        description: "Survivors of the Zariman colony ship who watch over the Angels.",
    ...
    },
    {
        ...
        location: "Relays",
        description: "The Sanctuary's keeper who catalogs all life in the Origin System.",
...
const SYNDICATE_RELATIONSHIPS: Record<string, { allied: string; opposed: string[] }> = {
    "steel-meridian": { allied: "red-veil", opposed: ["arbiters-of-hexis", "perrin-sequence"] },
    "arbiters-of-hexis": { allied: "cephalon-suda", opposed: ["steel-meridian", "red-veil"] },
    "cephalon-suda": { allied: "arbiters-of-hexis", opposed: ["perrin-sequence", "new-loka"] },
    "perrin-sequence": { allied: "new-loka", opposed: ["steel-meridian", "cephalon-suda"] },
    "red-veil": { allied: "steel-meridian", opposed: ["arbiters-of-hexis", "new-loka"] },
    "new-loka": { allied: "perrin-sequence", opposed: ["cephalon-suda", "red-veil"] },
};
...
const SYNDICATE_OFFERINGS: Record<string, { weapons: string[]; mods: string[] }> = {
    "steel-meridian": {
...
export function SyndicateView() {
    ...
    const [syndicateProgress, setSyndicateProgress] = useState<SyndicateProgress[]>(() => {
    ...
    const resetDailyStanding = () => {
        saveProgress(
            syndicateProgress.map((s) => ({ ...s, earnedToday: 0 }))
        );
    };

    const dailyCap = calculateDailyCap(masteryRank);

    const trackedSyndicates = syndicateProgress.map((prog) => {
        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-900/50 rounded-xl border border-purple-900/30 p-6">
                ...
                <div className="flex items-center gap-4 mb-6 p-4 bg-slate-800/50 rounded-lg border border-slate-700/50">
                    ...
                    <div className="ml-auto text-slate-400">
                        Daily Cap: <span className="text-cyan-400 font-bold">{dailyCap.toLocaleString()}</span>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    {trackedSyndicates.length === 0 ? (
                        ...
                        trackedSyndicates.map((item) => {
                            ...
                            const progressPercent = Math.min(100, (prog.currentStanding / targetCap) * 100);
                            const dailyProgressPercent = Math.min(100, (prog.earnedToday / dailyCap) * 100);
                                ...
                                >
                                    <div className="flex items-center justify-between mb-3">
                                        <div className="flex items-center gap-3">
                                            <h3 className={`font-bold ${syndicate.color}`}>{syndicate.name}</h3>
                                            <a
                                    ...
                                    <p className="text-sm text-slate-400 mb-4">{syndicate.description}</p>
                                    ...
                                    <div className="mb-4">
                                        <div className="flex items-center justify-between text-sm mb-1">
                                            ...
                                            <span className={syndicate.color}>
                                                {prog.currentStanding.toLocaleString()} / {targetCap.toLocaleString()}
                                            </span>
                                        </div>
                                        ...
                                        <div className="flex justify-between text-xs text-slate-500 mt-1">
                                            {STANDING_RANKS.map((rank, i) => (
                                                <span key={rank.name} className={i <= prog.targetRank ? syndicate.color : ""}>
                                                    {rank.name.slice(0, 3)}
                                                </span>
                                            ))}
                                        </div>
                                    ...
                                    <div className="mb-4">
                                        <div className="flex items-center justify-between text-sm mb-1">
                                            ...
                                            <span className="text-cyan-400">
                                                {prog.earnedToday.toLocaleString()} / {dailyCap.toLocaleString()}
                                            </span>
                                        </div>
                                        <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                                            <div
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <span className="text-sm text-slate-500">Add standing:</span>
                                    ...
                                    {SYNDICATE_RELATIONSHIPS[syndicate.id] && (
                                        <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-slate-700/50">
                                            <span className="text-xs text-green-400">
                                                Allied: {SYNDICATES.find(s => s.id === SYNDICATE_RELATIONSHIPS[syndicate.id].allied)?.name}
                                            </span>
                                            <span className="text-xs text-red-400">
                                                Opposed: {SYNDICATE_RELATIONSHIPS[syndicate.id].opposed.map(id =>
                                                    SYNDICATES.find(s => s.id === id)?.name
                                                ).join(", ")}
                                            </span>
                                        </div>
                                    )}
                                    ...
                                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-700/50">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => updateSyndicateStanding(syndicate.id, parseInt(e.target.value, 10) || 0)}
            ...
            {untrackedSyndicates.length > 0 && (
                <div className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-6">
                    <h3 className="text-lg font-bold text-slate-300 mb-4">Add Syndicate to Track</h3>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        {untrackedSyndicates.map((syndicate) => (
                            ...
                            >
                                <h4 className={`font-medium ${syndicate.color} mb-1`}>{syndicate.name}</h4>
                                <p className="text-xs text-slate-500 line-clamp-2">{syndicate.description}</p>
                            </button>
                        ))}
                    </div>
                </div>
            ...
            <div className="bg-slate-900/50 rounded-xl border border-amber-900/30 p-6">
                <h3 className="text-lg font-bold text-amber-400 mb-4">Notable Syndicate Offerings</h3>
                    ...
                    {SYNDICATES.map((syndicate) => {
                        ...
                        return (
                            <div
                                ...
                                className={`${syndicate.bgColor} rounded-lg border ${syndicate.borderColor} p-4`}
                            >
                                <h4 className={`font-medium ${syndicate.color} mb-3`}>{syndicate.name}</h4>
                                <div className="space-y-2">
                                    <div>
                                        <span className="text-xs text-slate-500 uppercase">Weapons</span>
                                    ...
                                    <div>
                                        <span className="text-xs text-slate-500 uppercase">Key Mods</span>
                                        <div className="flex flex-wrap gap-1 mt-1">
            ...
            {/* Syndicate Tips */}
            <div className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-6">
                <h3 className="text-lg font-bold text-slate-300 mb-4">Syndicate Tips</h3>
                <div className="grid gap-3 md:grid-cols-2">
                    <div className="bg-slate-800/30 rounded-lg p-3">
                        <h4 className="text-cyan-400 font-medium mb-1">📊 Daily Cap</h4>
                        <p className="text-sm text-slate-400">
                            Your daily standing cap is 16,000 + (500 × MR). Complete syndicate missions and wear sigils to earn standing.
                        </p>
                    </div>
                    <div className="bg-slate-800/30 rounded-lg p-3">
                        <h4 className="text-cyan-400 font-medium mb-1">⚔️ Allied Syndicates</h4>
                        <p className="text-sm text-slate-400">
                            Steel Meridian ↔ Red Veil. Hexis ↔ Suda. Perrin ↔ New Loka. Earning for one ally earns 50% for the other.
                        </p>
                    </div>
                    <div className="bg-slate-800/30 rounded-lg p-3">
                        <h4 className="text-cyan-400 font-medium mb-1">🎯 Medallions</h4>
                        <p className="text-sm text-slate-400">
                            Find medallions in syndicate missions. They give standing that doesn't count toward your daily cap!
                        </p>
                    </div>
                    <div className="bg-slate-800/30 rounded-lg p-3">
                        <h4 className="text-cyan-400 font-medium mb-1">🎖️ Pledging</h4>
                        <p className="text-sm text-slate-400">
                            Pledge allegiance at the Syndicate console to earn 15% of mission affinity as standing. Sigils are now cosmetic only.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-6">
                <h3 className="text-lg font-bold text-slate-300 mb-4">Open World & Other Syndicates</h3>
                <p className="text-sm text-slate-400 mb-4">
                    These syndicates have their own standing systems unrelated to the main 6.
                </p>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {OPEN_WORLD_SYNDICATES.map((syndicate) => (
                        ...
                        >
                            ...
                            <p className="text-xs text-slate-400 mt-1 line-clamp-2">{syndicate.description}</p>
                        </a>
                    ))}
                </div>
            </div>

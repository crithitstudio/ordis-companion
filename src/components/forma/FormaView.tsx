...
export function FormaView() {
  ...
  const [newPriority, setNewPriority] =
    useState<FormaItem["priority"]>("medium");
  ...
  const applyForma = useCallback(
    (id: string) => {
      ...
      updateItems(
        items.map((i) =>
          i.id === id
            ? { ...i, applied: Math.min(i.applied + 1, i.required) }
            : i,
        ),
      );
  ...
  const stats = useMemo(() => {
    ...
    const highPriority = items.filter(
      (i) => i.priority === "high" && i.applied < i.required,
    ).length;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-slate-900/50 rounded-xl border border-amber-700/30 p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <select
              value={newPriority}
              onChange={(e) =>
                setNewPriority(e.target.value as FormaItem["priority"])
              }
              className="bg-slate-800 border border-slate-600 rounded-lg px-3 py-2"
            >
              {PRIORITIES.map((p) => (
      ...
      {["high", "medium", "low"].map((priority) => {
        const priorityItems =
          groupedItems[priority as keyof typeof groupedItems];
        ...
        return (
          <div key={priority}>
            <h3
              className={`text-sm font-medium uppercase mb-2 flex items-center gap-2 ${priorityInfo.color.split(" ")[0]}`}
            >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span
                          className={`font-medium ${isComplete ? "text-slate-400 line-through" : "text-slate-200"}`}
                        >
                          {item.name}
                        </span>
      ...
      <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
        ...
        <ul className="text-sm text-slate-400 space-y-1">
          <li>
            • Forma blueprints drop from Void relics and take 24h to build
          </li>

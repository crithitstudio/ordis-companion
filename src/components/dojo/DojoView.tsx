...
const ROOM_COSTS: RoomCost[] = [
    { id: "cross-connector", name: "Cross Connector", category: "connector", credits: 500, forma: 0, resources: [{ name: "Ferrite", amount: 560 }] },
    { id: "t-connector", name: "T-Connector", category: "connector", credits: 500, forma: 0, resources: [{ name: "Ferrite", amount: 450 }] },
    { id: "straight-hallway", name: "Straight Hallway", category: "connector", credits: 500, forma: 0, resources: [{ name: "Ferrite", amount: 450 }] },
    { id: "elbow-connector", name: "Elbow Connector", category: "connector", credits: 500, forma: 0, resources: [{ name: "Ferrite", amount: 350 }] },
    { id: "tenno-lab", name: "Tenno Lab", category: "lab", credits: 50000, forma: 1, resources: [{ name: "Alloy Plate", amount: 2500 }, { name: "Rubedo", amount: 350 }] },
    { id: "energy-lab", name: "Energy Lab", category: "lab", credits: 50000, forma: 1, resources: [{ name: "Alloy Plate", amount: 2500 }, { name: "Circuits", amount: 350 }] },
    { id: "chem-lab", name: "Chem Lab", category: "lab", credits: 50000, forma: 1, resources: [{ name: "Alloy Plate", amount: 2500 }, { name: "Salvage", amount: 500 }] },
    { id: "bio-lab", name: "Bio Lab", category: "lab", credits: 50000, forma: 1, resources: [{ name: "Alloy Plate", amount: 2500 }, { name: "Nano Spores", amount: 1000 }] },
    { id: "orokin-lab", name: "Orokin Lab", category: "lab", credits: 100000, forma: 1, resources: [{ name: "Alloy Plate", amount: 5000 }, { name: "Control Module", amount: 4 }] },
    { id: "clan-great-hall", name: "Clan Great Hall", category: "hall", credits: 100000, forma: 1, resources: [{ name: "Ferrite", amount: 10000 }, { name: "Polymer Bundle", amount: 1000 }] },
    { id: "grandest-hall", name: "Grandest Hall", category: "hall", credits: 150000, forma: 1, resources: [{ name: "Ferrite", amount: 25000 }, { name: "Polymer Bundle", amount: 2500 }] },
    { id: "barracks", name: "Barracks", category: "utility", credits: 25000, forma: 1, resources: [{ name: "Ferrite", amount: 5000 }, { name: "Rubedo", amount: 250 }] },
    { id: "oracle", name: "Oracle", category: "utility", credits: 25000, forma: 1, resources: [{ name: "Ferrite", amount: 3500 }, { name: "Circuits", amount: 200 }] },
    { id: "dry-dock", name: "Dry Dock", category: "utility", credits: 100000, forma: 1, resources: [{ name: "Salvage", amount: 25000 }, { name: "Plastids", amount: 1500 }] },
    { id: "crimson-branch", name: "Crimson Branch", category: "utility", credits: 100000, forma: 1, resources: [{ name: "Alloy Plate", amount: 10000 }, { name: "Plastids", amount: 850 }] },
];
...
export function DojoView() {
    const [completedResearch, updateResearch] = useLocalStorageSet<string>("ordis-dojo-research");

    const toggleResearch = useCallback((itemId: string) => {
        updateResearch(prev => {
    ...
    const [expandedLabs, setExpandedLabs] = useState<Set<string>>(new Set(["tenno"]));
    const [filterType, setFilterType] = useState<string>("all");
    const [activeTab, setActiveTab] = useState<"research" | "rooms">("research");
    const [clanTier, setClanTier] = useState<string>("ghost");

    const tierMultiplier = CLAN_TIERS.find(t => t.id === clanTier)?.multiplier || 1;
    ...
    const getFilteredItems = useCallback((items: ResearchItem[]) => {
            ...
            <section className="bg-gradient-to-r from-orange-900/30 to-slate-900/50 rounded-xl border border-orange-700/30 p-6">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-right">
                        <div className="text-3xl font-bold text-orange-300">{stats.percentage}%</div>
                        <div className="text-slate-400 text-sm">{stats.completedCount}/{stats.totalItems} Complete</div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-5 gap-2 mt-4">
                    {stats.byLab.map(lab => {
                        const pct = lab.total > 0 ? Math.round((lab.completed / lab.total) * 100) : 0;
                        return (
                            <div key={lab.id} className="bg-slate-800/50 rounded-lg p-2 text-center">
                                <div className="text-xs text-slate-500 truncate">{lab.name.replace(" Lab", "")}</div>
                                <div className={`text-sm font-bold ${pct === 100 ? "text-green-400" : "text-slate-200"}`}>
                                    {pct}%
                                </div>
                            </div>
                        );
                ...
                <>
                    ...
                    <div className="space-y-3">
                        {RESEARCH_LABS.map(lab => {
                            ...
                            const labCompleted = labItems.filter(i => completedResearch.has(i.id)).length;

                            if (labItems.length === 0) return null;

                            return (
                                <div key={lab.id} className={`rounded-xl border overflow-hidden ${lab.color}`}>
                                    {/* Lab Header */}
                                    <button
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            {labCompleted === labItems.length && labItems.length > 0 && (
                                            ...
                                            {labItems.map(item => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`flex items-center gap-2 p-2 rounded-lg border transition-colors ${isComplete
                                                    ...
                                                    >
                                                        ...
                                                        <span className={`text-sm truncate ${isComplete ? "line-through opacity-60" : ""}`}>
                                                            {item.name}
                                                        </span>
                                                    </button>
                                                );
            ...
            {activeTab === "rooms" && (
                <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between gap-3">
                        <p className="text-slate-400 text-sm">Select your clan tier to see accurate costs</p>
                        <select
                            value={clanTier}
                    ...
                    {tierMultiplier > 1 && (
                        <div className="bg-orange-900/20 border border-orange-700/30 rounded-lg px-3 py-2 text-sm text-orange-300">
                            ⚡ Costs multiplied by {tierMultiplier}x for {CLAN_TIERS.find(t => t.id === clanTier)?.name} clan
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {ROOM_COSTS.map(room => (
                            <div key={room.id} className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-4">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="font-bold text-slate-200">{room.name}</div>
                                    <span className="text-xs px-2 py-0.5 rounded bg-slate-700 text-slate-400">{room.category}</span>
                                </div>
                                <div className="flex flex-wrap gap-3 text-sm">
                                    <div className="flex items-center gap-1">
                                        <span className="text-yellow-400">{(room.credits * tierMultiplier).toLocaleString()}</span>
                                        <span className="text-slate-500">Credits</span>
                                    </div>
                                    {room.forma > 0 && (
                                        <div className="flex items-center gap-1">
                                            <span className="text-cyan-400">{room.forma * tierMultiplier}</span>
                                ...
                                <div className="mt-2 flex flex-wrap gap-2">
                                    {room.resources.map(r => (
                                        <span key={r.name} className="text-xs px-2 py-1 bg-slate-800 rounded text-slate-300">

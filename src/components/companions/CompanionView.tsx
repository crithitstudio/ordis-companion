...
import { Heart, Plus, Trash2, Star, Shield, Zap, Eye } from "lucide-react";
...
const COMPANION_TYPES = [
  ...
  { id: "moa", label: "MOA", species: ["Lambeo", "Oloro", "Para"] },
  {
    id: "hound",
    label: "Hound",
    species: ["Bonewidow Hound", "Voidrig Hound", "Custom Hound"],
  },
  {
    id: "vulpaphyla",
    label: "Vulpaphyla",
    species: ["Crescent", "Panzer", "Sly"],
  },
  {
    id: "predasite",
    label: "Predasite",
    species: ["Pharaoh", "Vizier", "Medjay"],
  },
] as const;
...
export function CompanionView() {
  ...
  const stats = useMemo(
    () => ({
      ...
      pets: companions.filter((c) =>
        ["kubrow", "kavat", "vulpaphyla", "predasite"].includes(c.type),
      ).length,
      robotic: companions.filter((c) => ["moa", "hound"].includes(c.type))
        .length,
          ...
          <div className="bg-pink-900/20 border border-pink-700/30 rounded-lg p-3 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="font-bold text-pink-300">
                {activeCompanion.name} ({activeCompanion.species})
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-slate-900/50 rounded-xl border border-pink-700/30 p-4 space-y-3">
          ...
          <div className="flex gap-3">
            <select
              value={newType}
              onChange={(e) => {
                setNewType(e.target.value as Companion["type"]);
                setNewSpecies("");
              }}
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm text-slate-500">
                  {companion.species} •{" "}
                  {COMPANION_TYPES.find((t) => t.id === companion.type)?.label}
                </div>
              </div>
              ...
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateForma(companion.id, parseInt(e.target.value) || 0)
                  }
              ...
              <div className="flex gap-1">
                ...
                >
                  <Star
                    size={14}
                    className={companion.favorite ? "fill-current" : ""}
                  />
                </button>
                <button

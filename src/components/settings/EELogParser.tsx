    ...
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          setResults(parsed);
          addToast(
            `Parsed ${parsed.linesProcessed.toLocaleString()} lines`,
            "success",
          );
        } catch (err) {
          const message =
  ...
  return (
    <div className="bg-slate-900/50 rounded-xl border border-blue-900/30 p-6">
      ...
      <p className="text-sm text-slate-400 mb-4">
        Upload your Warframe EE.log file to extract trade history, mission
        rewards, and relic results.
      </p>
      ...
      <div className="bg-slate-800/50 rounded-lg p-3 mb-4 text-sm">
        ...
        <p className="text-xs text-slate-500 mt-2">
          Note: The log is overwritten each game session. Copy it before
          restarting the game.
        </p>
      </div>
      ...
      {!results ? (
          ...
          <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-600 rounded-lg cursor-pointer hover:border-blue-500 transition-colors bg-slate-800/30">
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              {isLoading ? (
                <RefreshCw
                  size={32}
                  className="text-blue-400 animate-spin mb-2"
                />
              ) : (
        ...
        <div className="space-y-4">
                ...
                {results.trades.map((trade, i) => (
                  ...
                  >
                    {trade.received.length > 0 && (
                      <div className="flex items-center gap-2 text-green-400">
                        ...
                        {trade.received.map((r, j) => (
                          <span
                            key={j}
                            className="bg-green-900/30 px-2 py-0.5 rounded text-xs"
                          >
                            {r.item}
                    ...
                    {trade.given.length > 0 && (
                      <div className="flex items-center gap-2 text-red-400 mt-1">
                        ...
                        {trade.given.map((g, j) => (
                          <span
                            key={j}
                            className="bg-red-900/30 px-2 py-0.5 rounded text-xs"
                          >
                            {g.item}
                    ...
                    {trade.platinum !== undefined && trade.platinum !== 0 && (
                      <div
                        className={`mt-1 text-xs ${trade.platinum > 0 ? "text-cyan-400" : "text-orange-400"}`}
                      >
                        Platinum: {trade.platinum > 0 ? "+" : ""}
                        {trade.platinum}
                      </div>
                    )}
                  </div>
          ...
          {results.trades.length === 0 &&
            results.relicResults.length === 0 &&
            results.missionRewards.length === 0 && (
              <div className="text-center py-4 text-slate-500">
                <AlertCircle size={24} className="mx-auto mb-2" />
                <p>No trades, relics, or mission data found in this log.</p>
                <p className="text-xs mt-1">
                  Try a different log file or play some missions first.
                </p>
              </div>
            )}

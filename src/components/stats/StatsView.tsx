    ...
    <div className="space-y-6">
      ...
      <section className="bg-gradient-to-r from-indigo-900/30 to-slate-900/50 rounded-xl border border-indigo-700/30 p-6">
        ...
        <div className="bg-slate-800/50 rounded-xl p-4">
          ...
          <div className="mt-2 text-sm text-slate-500 text-center">
            {overall.totalCompleted.toLocaleString()} /{" "}
            {overall.totalItems.toLocaleString()} items tracked
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {stats.map((stat) => {
          ...
          const percentage =
            stat.items > 0
              ? Math.round((stat.completed / stat.items) * 100)
              : 0;
      ...
      <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
        ...
        <ul className="text-sm text-slate-400 space-y-1">
          ...
          <li>
            • Use the various trackers to increase your completion percentage
          </li>
        </ul>
      </div>
    </div>

...
async function main() {
  ...
  writeFileSync(
    join(DATA_DIR, "sortieData.json"),
    JSON.stringify(sortieData, null, 2),
  );
  ...
  writeFileSync(
    join(DATA_DIR, "missionTypes.json"),
  ...
  console.log(
    "Saved itemNames.json (" + Object.keys(itemNames).length + " items)",
  );

...
const STEEL_ESSENCE_REWARDS = [
  {
    cost: 15,
    item: "Umbra Forma Blueprint",
    description: "Adds Umbra polarity",
  },
  { cost: 25, item: "Kuva (10,000)", description: "For Riven rerolls" },
  {
    cost: 15,
    item: "Primary Arcane Adapter",
    description: "Add arcane slot to primary",
  },
  {
    cost: 15,
    item: "Secondary Arcane Adapter",
    description: "Add arcane slot to secondary",
  },
...
export function SteelPathView() {
  ...
  const [wantedRewards, setWantedRewards] =
    useLocalStorageSet<string>("ordis-steel-wanted");
  ...
  const filteredNodes = useMemo(() => {
    ...
    if (missionTypeFilter !== "all") {
      nodes = nodes.filter((n) =>
        n.type.toLowerCase().includes(missionTypeFilter.toLowerCase()),
      );
    }
  ...
  const planetProgress = useMemo(() => {
    ...
    PLANETS.forEach((planet) => {
      ...
      const completed = planetNodes.filter((n) =>
        completedNodes.has(n.name),
      ).length;
    ...
    <div className="space-y-6">
          ...
          <div className="bg-slate-800/50 rounded-lg p-4">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                value={steelEssence}
                onChange={(e) =>
                  updateEssence(parseInt(e.target.value, 10) || 0)
                }
          ...
          {planetProgress.map(({ planet, completed, total }) => {
            ...
            return (
              <button
                key={planet}
                onClick={() =>
                  setSelectedPlanet(planet === selectedPlanet ? "all" : planet)
                }
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span
                    className={`text-sm font-medium ${isComplete ? "text-green-400" : "text-slate-300"}`}
                  >
                    {planet}
                  </span>
      ...
      <div className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-6">
        <h3 className="text-lg font-bold text-slate-300 mb-4 flex items-center gap-2">
          ...
          <span className="text-sm font-normal text-slate-500">
            ({filteredNodes.filter((n) => completedNodes.has(n.name)).length}/
            {filteredNodes.length} completed)
          </span>
        </h3>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span
                    className={`font-medium ${isCompleted ? "text-green-400" : "text-slate-300"}`}
                  >
                    {node.name}
                  </span>
                  {isCompleted && (
          ...
          {STEEL_ESSENCE_REWARDS.map((reward) => {
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span
                    className={`font-medium ${isWanted ? "text-yellow-400" : canAfford ? "text-purple-400" : "text-slate-400"}`}
                  >
                    {reward.item}
                  </span>
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      title={
                        isWanted ? "Remove from wishlist" : "Add to wishlist"
                      }
                    >
                      <Star
                        size={14}
                        className={isWanted ? "fill-yellow-400" : ""}
                      />
                    </button>
                    <span
                      className={`text-sm ${canAfford ? "text-purple-300" : "text-slate-500"}`}
                    >
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="bg-slate-800/30 rounded-lg p-3">
            ...
            <p className="text-sm text-slate-400">
              Steel Path enemies start at +100 levels with 2.5x health and
              shields. Bring your best builds!
            </p>
          </div>
          <div className="bg-slate-800/30 rounded-lg p-3">
            <h4 className="text-cyan-400 font-medium mb-1">💎 Steel Essence</h4>
            <p className="text-sm text-slate-400">
              Drops from Acolytes (5-7 each) and Steel Path Incursions. Farm
              during Acolyte spawns.
            </p>
          </div>
          <div className="bg-slate-800/30 rounded-lg p-3">
            <h4 className="text-cyan-400 font-medium mb-1">🎯 Incursions</h4>
            <p className="text-sm text-slate-400">
              Daily rotating missions with bonus Steel Essence rewards. Check
              the Teshin shop.
            </p>
          </div>
          <div className="bg-slate-800/30 rounded-lg p-3">
            ...
            <p className="text-sm text-slate-400">
              Completing all nodes unlocks the Steel Path Emote and permanent
              access to honors.
            </p>
          </div>
        </div>
      ...
      <div className="bg-slate-900/50 rounded-xl border border-cyan-900/30 p-6">
        ...
        <p className="text-sm text-slate-400 mb-4">
          Sold by Cavalero on weekly rotation in the Chrysalith. Buy adapters
          with Steel Essence.
        </p>
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {INCARNON_WEAPONS.map((week) => {
            ...
            return (
              ...
              >
                <h4
                  className={`text-xs uppercase mb-2 flex items-center gap-2 ${
                ...
                <div className="space-y-1">
                  {week.weapons.map((weapon) => (
                    <div
                      key={weapon}
                      className={`text-sm ${isCurrent ? "text-cyan-300" : "text-slate-300"}`}
                    >
                      {weapon}
                    </div>
                  ))}

...
export function RelicsView() {
    ...
    useEffect(() => {
        ...
        if (saved) setRelics(JSON.parse(saved));
    }, []);
    ...
    return (
        <div className="space-y-6">
            <div className="bg-slate-900/50 rounded-xl border border-amber-900/30 p-6">
                ...
                <p className="text-slate-400 mb-6">Track your void relic inventory and wishlist.</p>
                ...
                {filteredRelics.length === 0 ? (
                    ...
                    <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                        {filteredRelics.map(relic => (
                            <div key={relic.id} className={`bg-slate-800/30 rounded-lg p-4 border ${relic.wanted ? 'border-amber-500/50' : 'border-slate-700/50'}`}>
                                <div className="flex items-start justify-between mb-3">
                                    <div>
                                        <h4 className="text-slate-200 font-medium flex items-center gap-2">
                                            {relic.name}
                                            {relic.wanted && <Star size={14} className="text-amber-400 fill-amber-400" />}
                                        </h4>
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                    <select
                                        ...
                                        onChange={(e) => updateRelic(relic.id, { refinement: e.target.value as SavedRelic['refinement'] })}
                                    ...
                                    <div className="flex items-center gap-1">
                                        <button
                                            onClick={() => updateRelic(relic.id, { quantity: Math.max(0, relic.quantity - 1) })}
                                            className="px-2 py-1 bg-slate-700 hover:bg-slate-600 rounded text-slate-300"
                                            aria-label="Decrease quantity"
                                        >-</button>
                                        <span className="w-8 text-center text-slate-200">{relic.quantity}</span>
                                        <button
                                            onClick={() => updateRelic(relic.id, { quantity: relic.quantity + 1 })}
                                            className="px-2 py-1 bg-slate-700 hover:bg-slate-600 rounded text-slate-300"
                                ...
                                <button
                                    onClick={() => updateRelic(relic.id, { wanted: !relic.wanted })}

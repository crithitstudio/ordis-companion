...
interface ModSlot {
    ...
    rank: number;
    drain: number;
    polarity?: "madurai" | "vazarin" | "naramon" | "zenurik" | "unairu" | "penjaga" | "umbra";
}
...
const getAllMods = () => {
    ...
    Object.values(itemsData).forEach(item => {
        ...
        const modItem = item as { name?: string; type?: string; category?: string; baseDrain?: number; polarity?: string };
        if ((modItem.type && modItem.type.includes("Mod")) || modItem.category === "Mods") {
            if (modItem.name) {
                mods.push({
                    name: modItem.name,
...
export function BuildView() {
    ...
    // Mod search state
    const [activeModSearch, setActiveModSearch] = useState<{ buildId: string; slotIndex: number; query: string } | null>(null);
    ...
    const createBuild = useCallback(() => {
        ...
        const build: Build = {
            ...
            mods: Array(8).fill(null).map(() => ({ name: "", rank: 0, drain: 0 })),
            capacity: 0,
            maxCapacity: 60,
            forma: 0,
            notes: "",
    ...
    const toggleFavorite = useCallback((id: string) => {
        updateBuilds(builds.map(b =>
            b.id === id ? { ...b, favorite: !b.favorite } : b
        ));
    }, [builds, updateBuilds]);

    // Duplicate build
    const duplicateBuild = useCallback((build: Build) => {
    ...
    // Update mod slot
    const updateModSlot = useCallback((buildId: string, slotIndex: number, mod: Partial<ModSlot>) => {
        updateBuilds(builds.map(b => {
    ...
    const stats = useMemo(() => ({
        ...
        weapons: builds.filter(b => ["primary", "secondary", "melee"].includes(b.type)).length,
        ...
        <div className="space-y-6">
            ...
            <section className="bg-gradient-to-r from-emerald-900/30 to-slate-900/50 rounded-xl border border-emerald-700/30 p-6">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="text-right">
                            <div className="text-3xl font-bold text-emerald-300">{stats.total}</div>
                            <div className="text-slate-400 text-sm">Saved Builds</div>
                        </div>
                        <button onClick={exportBuilds} className="p-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-slate-300" title="Export Builds">
                            <Download size={18} />
                        </button>
                        <button onClick={importBuilds} className="p-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-slate-300" title="Import Builds">
                            <Upload size={18} />
                        </button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-3">
                    <div className="bg-slate-800/50 rounded-lg p-2 text-center">
                        ...
                        <div className="text-lg font-bold text-slate-200">{stats.warframes}</div>
                    </div>
                    <div className="bg-slate-800/50 rounded-lg p-2 text-center">
                        <div className="text-xs text-slate-500">Weapons</div>
                        <div className="text-lg font-bold text-slate-200">{stats.weapons}</div>
                    </div>
                    <div className="bg-slate-800/50 rounded-lg p-2 text-center">
                    ...
                    <div className="bg-slate-800/50 rounded-lg p-2 text-center">
                        ...
                        <div className="text-lg font-bold text-emerald-300">{stats.total}</div>
                    </div>
                </div>
            </section>
            ...
            <div className="space-y-3">
                {filteredBuilds.length === 0 ? (
                    ...
                    filteredBuilds.map(build => {
                        ...
                        const TypeIcon = BUILD_TYPES.find(t => t.id === build.type)?.icon || Layers;

                        return (
                            <div
                                key={build.id}
                                className={`bg-slate-900/50 rounded-xl border overflow-hidden transition-all ${build.favorite ? "border-yellow-600/50" : "border-slate-700/30"
                                    }`}
                            >
                                {/* Build Header */}
                                <button
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-left">
                                            <div className="font-bold text-slate-200 flex items-center gap-2">
                                                {build.name}
                                                {build.favorite && <Star size={14} className="text-yellow-400 fill-yellow-400" />}
                                            </div>
                                            <div className="text-sm text-slate-400">{build.itemName}</div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <span className={`text-sm font-medium ${build.capacity > build.maxCapacity ? "text-red-400" :
                                            build.capacity > build.maxCapacity * 0.9 ? "text-yellow-400" :
                                                "text-slate-500"
                                            }`}>
                                    ...
                                    <div className="p-4 border-t border-slate-700/50">
                                        ...
                                        <div className="mb-4">
                                            <div className="flex justify-between text-xs mb-1">
                                                ...
                                                <span className={build.capacity > build.maxCapacity ? "text-red-400" : "text-slate-400"}>
                                                    {build.capacity}/{build.maxCapacity}
                                                </span>
                                            </div>
                                            <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full transition-all ${build.capacity > build.maxCapacity ? "bg-red-500" :
                                                        build.capacity > build.maxCapacity * 0.9 ? "bg-yellow-500" :
                                                            "bg-emerald-500"
                                                        }`}
                                                    style={{ width: `${Math.min((build.capacity / build.maxCapacity) * 100, 100)}%` }}
                                                />
                                            </div>
                                            {build.capacity > build.maxCapacity && (
                                                <p className="text-xs text-red-400 mt-1">⚠ Over capacity by {build.capacity - build.maxCapacity} - add Forma or remove mods</p>
                                            )}
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
                                            {build.mods.map((mod, i) => {
                                                const isSearching = activeModSearch?.buildId === build.id && activeModSearch?.slotIndex === i;
                                                const filteredMods = isSearching && activeModSearch.query.length >= 2
                                                    ? ALL_MODS.filter(m => m.name.toLowerCase().includes(activeModSearch.query.toLowerCase())).slice(0, 8)
                                                    : [];

                                                return (
                                                    <div
                                                        key={i}
                                                        className={`relative p-2 rounded-lg border ${mod.name
                                                            ? "bg-slate-800/50 border-emerald-700/30"
                                                            : "bg-slate-800/30 border-slate-700/50 border-dashed"
                                                            }`}
                                                    >
                                                        <div className="relative">
                                                            <Search size={12} className="absolute left-0 top-1/2 -translate-y-1/2 text-slate-500" />
                                                            <input
                                                                type="text"
                                                                value={isSearching ? activeModSearch.query : mod.name}
                                                                onChange={e => {
                                                                    if (!isSearching) {
                                                                        setActiveModSearch({ buildId: build.id, slotIndex: i, query: e.target.value });
                                                                    } else {
                                                                        setActiveModSearch({ ...activeModSearch, query: e.target.value });
                                                                    }
                                                                }}
                                                                onFocus={() => setActiveModSearch({ buildId: build.id, slotIndex: i, query: mod.name })}
                                                                onBlur={() => setTimeout(() => setActiveModSearch(null), 200)}
                                                            ...
                                                            <div className="absolute left-0 right-0 top-full mt-1 bg-slate-800 border border-slate-600 rounded-lg shadow-xl z-20 max-h-40 overflow-y-auto">
                                                                {filteredMods.map(m => (
                                                                    <button
                                                                        key={m.name}
                                                                        onMouseDown={() => {
                                                                            updateModSlot(build.id, i, { name: m.name, drain: m.drain });
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <span className="text-slate-500 ml-1">{m.drain}</span>
                                                                    </button>
                                                                ))}
                                                            </div>
                                                        )}
                                                        ...
                                                        <div className="flex gap-2 mt-1">
                                                            <input
                                                                type="number"
                                                                value={mod.drain || ""}
                                                                onChange={e => updateModSlot(build.id, i, { drain: parseInt(e.target.value) || 0 })}
                                                                placeholder="Drain"
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                className={`p-2 rounded-lg transition-colors ${build.favorite ? "bg-yellow-600/20 text-yellow-400" : "bg-slate-700 text-slate-400"
                                                    }`}
                                                title="Toggle favorite"
                                            >
                                                <Star size={16} className={build.favorite ? "fill-current" : ""} />
                                            </button>
                                            <button
            ...
            <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
                ...
                <ul className="text-sm text-slate-400 space-y-1">
                    <li>• Use <strong>Overframe</strong> or <strong>Overframe.gg</strong> for detailed build optimization</li>

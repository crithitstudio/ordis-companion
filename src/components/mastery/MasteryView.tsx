...
export function MasteryView() {
  ...
  const masteredCount = masterableItems.filter((i) =>
    masteredItems.has(i.path),
  ).length;
  ...
  const categoryStats = useMemo(() => {
    ...
    masterableItems.forEach((item) => {
      if (!stats[item.category])
        stats[item.category] = { total: 0, mastered: 0 };
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div
                    className={`font-medium truncate ${mastered ? "text-slate-400 line-through" : "text-slate-200"}`}
                  >
                    {item.name}
                  </div>

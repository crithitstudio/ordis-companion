...
function parseItemWithQuantity(text: string): { item: string; quantity: number } {
    ...
    if (parenMatch) {
        return { item: parenMatch[1].trim(), quantity: parseInt(parenMatch[2], 10) };
    }
...
function parseLogLine(line: string): { timestamp: number; source: string; type: string; message: string } | null {
...
export function parseEELog(logContent: string): EELogParseResult {
    ...
    for (const line of lines) {
        ...
        try {
            ...
            if (currentTrade) {
                ...
                for (const match of receivedMatches) {
                    ...
                    if (parsed.item.toLowerCase().includes("platinum")) {
                        ...
                        if (platMatch) {
                            currentTrade.platinum = (currentTrade.platinum || 0) + parseInt(platMatch[1], 10);
                        }
                    } else {
            ...
            // Mission completion
            if (PATTERNS.missionComplete.test(message) || PATTERNS.endOfMission.test(message)) {
    ...
    if (currentTrade && (currentTrade.given.length > 0 || currentTrade.received.length > 0)) {
        result.trades.push(currentTrade);
    }
    if (currentMission && currentMission.rewards.length > 0) {
        result.missionRewards.push(currentMission);
    ...
    if (results.trades.length > 0) {
        ...
        results.trades.forEach((trade, i) => {
            ...
            if (trade.received.length > 0) {
                lines.push(`Received: ${trade.received.map(r => `${r.item}${r.quantity > 1 ? ` x${r.quantity}` : ""}`).join(", ")}`);
            }
            if (trade.given.length > 0) {
                lines.push(`Given: ${trade.given.map(g => `${g.item}${g.quantity > 1 ? ` x${g.quantity}` : ""}`).join(", ")}`);
            }
            if (trade.platinum) {
                lines.push(`Platinum: ${trade.platinum > 0 ? "+" : ""}${trade.platinum}`);
            }
            lines.push("");
        });
    }

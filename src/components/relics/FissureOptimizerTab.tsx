...
function getRelicDrops(): Map<string, { item: string; rarity: string; chance: number }[]> {
    const relicDrops = new Map<string, { item: string; rarity: string; chance: number }[]>();
        ...
        if (item.type === "Relic" || item.category === "Relics") {
            ...
            item.rewards?.forEach((r: any, i: number) => {
                ...
                if (itemName) {
                    ...
                    const chance = rarity === "Rare" ? 2 : rarity === "Uncommon" ? 11 : 25.33;
...
export function FissureOptimizerTab({ fissures }: FissureOptimizerProps) {
    ...
    const optimalRuns = useMemo<OptimalRun[]>(() => {
        ...
        return fissures.map(fissure => {
            ...
            const relicsWithValue = matchingRelics.map((relic: AlecaRelic) => {
            ...
            const totalRelics = relicsWithValue.reduce((sum: number, r: { count: number }) => sum + r.count, 0);
            const avgValue = totalRelics > 0
                ? relicsWithValue.reduce((sum: number, r: { value: number; count: number }) => sum + r.value * r.count, 0) / totalRelics
                : 0;
            ...
            .sort((a, b) => {
                switch (sortBy) {
                    ...
                    case "count": return b.totalRelics - a.totalRelics;
    ...
    if (fissures.length === 0) {
        return (
            <div className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-8 text-center">
                ...
                <h3 className="text-lg font-bold text-slate-300 mb-2">No Active Fissures</h3>
                <p className="text-slate-400">
                    Waiting for world state data...
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            {bestRun && (
                <div className="bg-gradient-to-r from-cyan-900/40 to-slate-900/50 rounded-xl border border-cyan-700/40 p-6">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-cyan-400">Best Fissure to Run</h2>
                    ...
                    <div className="bg-slate-800/50 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                            <div>
                                ...
                                <div className="text-sm text-slate-400">
                                    {bestRun.fissure.tier} • {bestRun.fissure.missionType} • {bestRun.fissure.enemy}
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-4">
                            <div className="text-center bg-slate-900/50 rounded-lg p-2">
                                <div className="text-xl font-bold text-slate-200">{bestRun.totalRelics}</div>
                                <div className="text-xs text-slate-500">Owned Relics</div>
                            </div>
                            <div className="text-center bg-slate-900/50 rounded-lg p-2">
                                <div className="text-xl font-bold text-purple-400">{bestRun.avgValue}p</div>
                        ...
                        {/* Top relics to bring */}
                        <div className="text-sm text-slate-400 mb-2">Best relics to bring:</div>
            ...
            <div className="space-y-3">
                <h3 className="text-lg font-bold text-slate-300">All Matching Fissures</h3>
                ...
                {optimalRuns.length === 0 ? (
                    ...
                    optimalRuns.map((run, i) => (
                        <div
                            key={run.fissure.id}
                            className={`bg-slate-900/50 rounded-lg border p-4 transition-colors ${i === 0 ? "border-cyan-700/50" : "border-slate-700/30 hover:border-slate-600/50"
                                }`}
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className={`text-center min-w-[50px] ${i === 0 ? "text-cyan-400" : "text-slate-400"}`}>
                                    ...
                                    <div>
                                        ...
                                        <div className="text-sm text-slate-400">
                                            {run.fissure.missionType} • {run.fissure.enemy} • {run.fissure.eta}
                                        </div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-slate-300 font-medium">{run.totalRelics} relics</div>
                                    <div className="text-sm text-purple-400">{run.avgValue}p avg</div>
                                </div>
                            </div>
                        </div>
                    ))
            ...
            <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
                ...
                <ul className="text-sm text-slate-400 space-y-1">
                    <li>• <strong>Mission Speed</strong>: Capture/Exterminate are fastest</li>
                    <li>• <strong>Relic Count</strong>: More relics = more runs before needing to refarm</li>
                    <li>• <strong>Avg Value</strong>: Expected platinum return per relic</li>

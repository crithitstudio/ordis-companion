...
export async function fetchRelics(
  publicToken: string,
): Promise<AlecaFrameData> {
  try {
    const response = await fetch(
      `${ALECA_API_BASE}/relics?token=${encodeURIComponent(publicToken)}`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        throw new Error(
          'Relic data not found - ensure "relics" access is enabled for this token',
        );
      }

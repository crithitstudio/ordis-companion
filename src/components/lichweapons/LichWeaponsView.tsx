...
export function LichWeaponsView() {
  ...
  const addWeapon = useCallback(
    (weaponName: string, type: "kuva" | "tenet") => {
  ...
  // Add Coda weapon
  const addCodaWeapon = useCallback(
    (weaponName: string) => {
  ...
  const removeWeapon = useCallback(
    (weaponName: string, type: TabType) => {
  ...
  const updateBonus = useCallback(
    (weaponName: string, type: "kuva" | "tenet", newBonusValue: number) => {
      const newState = { ...state };
      newState[type] = newState[type].map((w) =>
        w.name === weaponName
          ? {
              ...w,
              bonus: newBonusValue,
              valenceCount: w.valenceCount + 1,
              maxBonus: newBonusValue >= 60,
            }
          : w,
      );
      updateState(newState);
  ...
  const updateForma = useCallback(
    (weaponName: string, delta: number) => {
      const newState = { ...state };
      newState.coda = newState.coda.map((w) =>
        w.name === weaponName
          ? {
              ...w,
              formaCount: Math.max(0, Math.min(5, w.formaCount + delta)),
              maxForma: w.formaCount + delta >= 5,
            }
          : w,
      );
      updateState(newState);
  ...
  const stats = useMemo(() => {
    ...
    return {
      kuvaOwned,
      kuvaTotal: KUVA_WEAPONS.length,
      tenetOwned,
      tenetTotal: TENET_WEAPONS.length,
      codaOwned,
      codaTotal: CODA_WEAPONS.length,
      kuvaMaxed,
      tenetMaxed,
      codaMaxed,
      totalOwned: kuvaOwned + tenetOwned + codaOwned,
      totalWeapons:
        KUVA_WEAPONS.length + TENET_WEAPONS.length + CODA_WEAPONS.length,
    };
  }, [state]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {weapons.map((weapon) => {
            const ownedKT = !isCoda
              ? state[activeTab as "kuva" | "tenet"].find(
                  (w) => w.name === weapon,
                )
              : null;
            const ownedCoda = isCoda
              ? state.coda.find((w) => w.name === weapon)
              : null;
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`font-medium ${owned ? "text-slate-200" : "text-slate-400"}`}
                    >
                      {displayName}
                    </span>
                  </div>
                  ...
                  {ownedKT && (
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-xs px-2 py-0.5 rounded ${ownedKT.maxBonus ? "bg-yellow-600/30 text-yellow-300" : "bg-slate-700 text-slate-300"}`}
                      >
                        {ownedKT.bonus}%
                      </span>
                  ...
                  {ownedCoda && (
                    <div className="flex items-center gap-1">
                      {[1, 2, 3, 4, 5].map((i) => (
                        <div
                          key={i}
                          className={`w-2 h-4 rounded-sm ${i <= ownedCoda.formaCount ? "bg-yellow-400" : "bg-slate-700"}`}
                        />
                      ))}
                    </div>
                  )}
                ...
                {isAdding ? (
                  isCoda ? (
                    <div className="mt-3 flex gap-2">
                      <button
                        onClick={() => addCodaWeapon(weapon)}
                        className="flex-1 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-500"
                      >
                        Add
                      </button>
                      <button
                        onClick={() => setAddingWeapon(null)}
                    ...
                    <div className="mt-3 space-y-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
                            addWeapon(weapon, activeTab as "kuva" | "tenet")
                          }
                          className="flex-1 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-500"
                        >
                          Add
                  ...
                  <div className="mt-2 flex gap-2">
                    {ownedKT && !ownedKT.maxBonus && (
                      <button
                        onClick={() =>
                          updateBonus(
                            weapon,
                            activeTab as "kuva" | "tenet",
                            Math.min(60, ownedKT.bonus + 5),
                          )
                        }
                        className="flex-1 py-1 bg-slate-700 text-slate-300 text-xs rounded hover:bg-slate-600"
                      >
                        Valence +5%
                      </button>
                    )}
                    ...
                    <button
                      onClick={() => removeWeapon(weapon, activeTab)}
                      className="px-2 py-1 bg-red-900/50 text-red-300 text-xs rounded hover:bg-red-900"
                    >
                      Remove
                    </button>
                  </div>
                  ...
                  <button
                    onClick={() => setAddingWeapon(weapon)}
                    className="mt-2 w-full py-1 bg-slate-700 text-slate-300 text-sm rounded hover:bg-slate-600"
                  >
                    + Add
                  </button>
                )}
      ...
      {!isCoda && (
        <div className="bg-slate-900/50 rounded-xl border border-yellow-700/30 p-4">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setFusionHigh(
                    Math.min(60, Math.max(25, parseInt(e.target.value) || 25)),
                  )
                }
                min={25}
                max={60}
            ...
            <span className="text-slate-500">=</span>
            <div
              className={`text-2xl font-bold px-4 py-1 rounded ${fusionResult >= 60 ? "text-yellow-300 bg-yellow-900/30" : "text-cyan-400 bg-slate-700"}`}
            >
              {fusionResult}%
            </div>
            {fusionResult >= 60 && (
          ...
          <p className="text-xs text-slate-500 mt-2">
            Formula: Higher + (Lower × 10%), capped at 60%
          </p>
        </div>
      )}
      ...
      <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
        ...
        <ul className="text-sm text-slate-400 space-y-1">
          {activeTab === "coda" ? (
            <>
              <li>
                • Coda weapons are purchased from Eleanor using{" "}
                <strong>Live Heartcells</strong>
              </li>
              <li>
                • Use 5 Forma to reach 80 mod capacity (like Prime weapons)
              </li>
              <li>
                • <strong>Elemental Vice</strong> lets you change element at max
                level
              </li>
              <li>
                • Farm Heartcells by defeating Technocyte Codas in Hollvania
              </li>
            </>
          ) : (
            <>
              ...
              <li>
                • <strong>Heat</strong> and <strong>Toxin</strong> are generally
                best
              </li>

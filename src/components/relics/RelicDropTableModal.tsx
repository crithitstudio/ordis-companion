...
import { Search, Gem, ChevronDown, ChevronUp, ExternalLink, X } from 'lucide-react';
...
interface PrimeDrop {
    ...
    imageName?: string;
    uniqueName?: string;
    sources: { relic: string; refinement: string; rarity: string; chance: number }[];
}
...
export function RelicDropTableModal({ isOpen, onClose }: RelicDropTableModalProps) {
    ...
    const filteredItems = useMemo(() => {
        ...
        if (searchMode === 'item') {
            return primeDrops.filter(p => p.item.toLowerCase().includes(q)).slice(0, 50);
        } else {
            return relicContents.filter(r => r.name.toLowerCase().includes(q)).slice(0, 50);
        }
    ...
    const getRarityColor = (rarity: string) => {
        switch (rarity.toLowerCase()) {
            case 'common': return 'text-amber-200 bg-amber-900/30';
            case 'uncommon': return 'text-slate-200 bg-slate-600/50';
            case 'rare': return 'text-yellow-300 bg-yellow-900/50';
    ...
    const getEraColor = (era: string) => {
        switch (era.toLowerCase()) {
            case 'lith': return 'text-amber-300';
            case 'meso': return 'text-slate-300';
            case 'neo': return 'text-blue-300';
            case 'axi': return 'text-yellow-400';
            case 'requiem': return 'text-purple-400';
            ...
            >
                ...
                <div className="p-4 flex flex-col gap-4 overflow-hidden">
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={() => { setSearchMode('item'); setExpandedItem(null); }}
                        ...
                        <button
                            onClick={() => { setSearchMode('relic'); setExpandedItem(null); }}
                    ...
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder={searchMode === 'item' ? "Search Prime parts (e.g. Rhino, Neuroptics)..." : "Search relics (e.g. Lith A1, Axi)..."}
                            value={searchQuery}
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 max-h-[50vh]">
                        {searchQuery.length < 2 ? (
                            ...
                            <div className="text-center text-slate-500 py-8">
                                ...
                                <p className="text-xs mt-2">Try searching for Prime parts like "Rhino", "Neuroptics", or "Barrel"</p>
                            </div>
                        ) : searchMode === 'item' ? (
                            // Item search results
                                ...
                                >
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            {item.uniqueName && getItemImageUrl({ uniqueName: item.uniqueName }) && (
                                                <img
                                                    src={getItemImageUrl({ uniqueName: item.uniqueName }) || ''}
                                                    alt=""
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-slate-500">{item.sources.length} sources</span>
                                            {expandedItem === item.item ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                                        </div>
                                    </button>
                                    {expandedItem === item.item && (
                                        <div className="px-3 pb-3 space-y-1">
                                            {item.sources.slice(0, 20).map((src, i) => (
                                                <div key={i} className="flex items-center justify-between py-1 px-2 bg-slate-900/50 rounded text-sm">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-slate-300">{src.relic}</span>
                                                        <span className="text-xs text-slate-500">({src.refinement})</span>
                                                    </div>
                                                    <span className={`px-2 py-0.5 rounded text-xs ${getRarityColor(src.rarity)}`}>
                                                        {src.rarity}
                                                    </span>
                                                </div>
                                            ))}
                            ...
                            (filteredItems as RelicInfo[]).map(relic => (
                                ...
                                >
                                    <button
                                        onClick={() => setExpandedItem(expandedItem === relic.name ? null : relic.name)}
                                        className="w-full p-3 flex items-center justify-between text-left"
                                    >
                                        <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-slate-500">{relic.rewards.length} rewards</span>
                                            {expandedItem === relic.name ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                                        </div>
                                    </button>
                                        ...
                                        <div className="px-3 pb-3 space-y-1">
                                            {relic.rewards.map((reward, i) => (
                                                <div key={i} className="flex items-center justify-between py-1 px-2 bg-slate-900/50 rounded text-sm">
                                                    ...
                                                    <span className={`px-2 py-0.5 rounded text-xs ${getRarityColor(reward.rarity)}`}>
                                                        {reward.rarity}
                                                    </span>
                                                </div>
                                            ))}

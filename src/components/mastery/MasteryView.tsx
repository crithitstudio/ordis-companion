...
// Tab options
type MasteryTab = "equipment" | "starchart" | "junctions" | "intrinsics" | "recommendations";
...
export function MasteryView() {
  ...
  const [completedNodes, setCompletedNodes] = useLocalStorageSet<string>("ordis-mastery-nodes");
  const [completedJunctions, setCompletedJunctions] = useLocalStorageSet<string>("ordis-mastery-junctions");
  ...
  const updateIntrinsicRank = useCallback((name: string, rank: number) => {
    setIntrinsicRanks(prev => {
      const updated = { ...prev, [name]: Math.max(0, Math.min(MAX_INTRINSIC_RANK, rank)) };
  ...
  const masterableItems = useMemo(() => {
    ...
    Object.entries(itemsData).forEach(([path, data]) => {
      ...
      if (
        ...
        (uniqueName.includes("/KitGun") && !uniqueName.includes("Barrel") && !uniqueName.includes("Chamber")) || // Allow Barrels and Chambers (Kitguns)
        (uniqueName.includes("/ModularMelee") && !uniqueName.includes("Tip")) ||
        (data.category === "Misc" && type !== "Amp" && type !== "K-Drive Component" && !uniqueName.includes("Hoverboard") && !uniqueName.includes("MoaPet") && !uniqueName.includes("HoundPet")) ||
        // Exclude Amp Parts (Brace/Scaffold) - Keep Prisms
        (data.category === "Amp" && (data.name.includes("Brace") || data.name.includes("Scaffold"))) ||
        (type === "Amp" && (data.name.includes("Brace") || data.name.includes("Scaffold"))) ||
        // Exclude Dev/Test Items
        ["Uriel", "Nokko", "Oraxia", "Tink", "Zodian"].some(bad => data.name.includes(bad)) ||
        // Exclude Modular Pet Parts (Mutagens, Antigens, Gyros, Brackets, Cores)
        data.name.includes("Mutagen") ||
        data.name.includes("Antigen") ||
        (uniqueName.includes("/MoaPetParts/") && !uniqueName.includes("MoaPetHead")) || // Only Head is masterable for MOAs
        (uniqueName.includes("/ZanukaPetParts/") && !uniqueName.includes("ZanukaPetPartHead")) // Only Head is masterable for Hounds
      ) {
        return; // Skip
      }
      ...
      if (uniqueName.includes("/EntratiMech/")) {
        ...
        cat = "Sentinel Weapons";
      } else if (uniqueName.includes("/KubrowPet/") || uniqueName.includes("/CatbrowPet/") || uniqueName.includes("/PredasitePet/") || uniqueName.includes("/VulpaphylaPet/")) {
        cat = "Companions";
      } else if (uniqueName.includes("MoaPet") || uniqueName.includes("/ZanukaPet/") || uniqueName.includes("/Robot/") || data.type === "Hound" || uniqueName.includes("HoundPet") || uniqueName.includes("/Hounds/")) {
        cat = "Robotic";
      } else if (data.category === "Pets") {
        cat = "Companions";
      } else if (uniqueName.includes("/KDrive/Parts/Board") || (uniqueName.includes("Hoverboard") && uniqueName.includes("Deck"))) {
        cat = "K-Drive";
      } else if ((uniqueName.includes("/Operator/Amp/") || uniqueName.includes("OperatorAmp")) && (data.name.includes("Prism") || uniqueName.includes("Prism") || data.type === "Amp" || data.name === "Mote Amp")) {
        cat = "Amp";
      ...
      if (MASTERABLE_CATEGORIES.includes(cat)) {
        ...
        if (cat === "Sentinel Weapons" || (data.category === "Robotic" && type.includes("Weapon"))) {
          xp = 3000;
        }
        ...
        if (data.name === "Paracesis" || data.name.startsWith("Kuva ") || data.name.startsWith("Tenet ") || uniqueName.includes("/Necramech/")) {
          xp = 4000;
  ...
  const xpStats = useMemo(() => {
    const categoryXP: Record<string, { total: number; mastered: number; count: number; masteredCount: number }> = {};

    masterableItems.forEach((item) => {
      if (!categoryXP[item.category]) {
        categoryXP[item.category] = { total: 0, mastered: 0, count: 0, masteredCount: 0 };
      }
    ...
    const totalEquipmentXP = Object.values(categoryXP).reduce((sum, c) => sum + c.total, 0);
    const masteredEquipmentXP = Object.values(categoryXP).reduce((sum, c) => sum + c.mastered, 0);
    ...
    const totalIntrinsicsRanks = allIntrinsics.reduce((sum, name) => sum + (intrinsicRanks[name] || 0), 0);
    const intrinsicsXP = totalIntrinsicsRanks * INTRINSIC_XP;
    const maxIntrinsicsXP = allIntrinsics.length * MAX_INTRINSIC_RANK * INTRINSIC_XP;

    const totalXP = masteredEquipmentXP + nodesXP + junctionsXP + intrinsicsXP;
    const maxXP = totalEquipmentXP + totalNodesXP + totalJunctionsXP + maxIntrinsicsXP;
  ...
  }, [masterableItems, masteredItems, completedNodes, completedJunctions, starChartNodes, intrinsicRanks]);
      ...
      <section className="bg-gradient-to-r from-cyan-900/30 to-slate-900/50 rounded-xl border border-cyan-700/30 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            <div className={`text-4xl font-bold ${mrInfo.isLegendary ? 'text-yellow-400' : 'text-cyan-300'}`}>{mrInfo.displayRank}</div>
            <div className="text-slate-400 text-sm">
              {xpStats.totalXP.toLocaleString()} / {xpStats.maxXP.toLocaleString()} XP
            </div>
          </div>
        </div>
        ...
        <div className="mb-4">
          <div className="flex justify-between text-xs text-slate-500 mb-1">
            <span>Progress to {mrInfo.rank >= 30 ? `LR ${mrInfo.rank - 30 + 1}` : `MR ${mrInfo.rank + 1}`}</span>
        ...
        <div className="grid grid-cols-4 gap-4 text-center">
          <div className="bg-slate-800/50 rounded-lg p-3">
            <div className="text-xs text-slate-500 uppercase mb-1">Equipment</div>
          ...
          <div className="bg-slate-800/50 rounded-lg p-3">
            <div className="text-xs text-slate-500 uppercase mb-1">Star Chart</div>
            <div className="text-lg font-bold text-cyan-300">
          ...
          <div className="bg-slate-800/50 rounded-lg p-3">
            <div className="text-xs text-slate-500 uppercase mb-1">Junctions</div>
            <div className="text-lg font-bold text-cyan-300">
          ...
          <div className="bg-slate-800/50 rounded-lg p-3">
            <div className="text-xs text-slate-500 uppercase mb-1">Intrinsics</div>
            <div className="text-lg font-bold text-cyan-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-[50vh] overflow-y-auto">
            {filteredItems.slice(0, 150).map((item) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${mastered ? "bg-green-600 border-green-600" : "border-slate-500"
                      }`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div
                      className={`font-medium truncate text-sm ${mastered ? "text-slate-400 line-through" : "text-slate-200"
                        }`}
                    >
                      {item.name}
                    </div>
                    <div className="text-xs text-slate-500 flex justify-between">
                      ...
                      <span className="text-cyan-400">{item.xp.toLocaleString()} XP</span>
                    </div>
                  </div>
                </button>
              );
        ...
        <section className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm text-slate-400">
              {completedNodes.size} / {xpStats.totalNodeCount} nodes ({NODE_XP} XP each)
            </div>
          </div>
          <p className="text-xs text-slate-500 mb-4 italic">
            Note: Some newer regions (1999 Höllvania, etc.) may not be included in the data.
          </p>
          <div className="space-y-2 max-h-[60vh] overflow-y-auto">
            {Array.from(starChartNodes.entries())
              .sort(([a], [b]) => a.localeCompare(b))
              .map(([planet, nodes]: [string, { name: string; key: string }[]]) => {
                const completedCount = nodes.filter((n) => completedNodes.has(n.key)).length;
                const isExpanded = expandedPlanet === planet;
                return (
                  <div key={planet} className="bg-slate-800/50 rounded-lg overflow-hidden">
                    <button
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="font-medium text-slate-200">{planet}</span>
      ...
      {activeTab === "junctions" && (
        <section className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm text-slate-400">
              {completedJunctions.size} / {JUNCTIONS.length} ({JUNCTION_XP.toLocaleString()} XP each)
            </div>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className={`font-medium ${completed ? "text-green-400" : "text-slate-200"}`}>
                      {junction.name}
                    </div>
                    <div className="text-xs text-slate-500">
      ...
      {activeTab === "intrinsics" && (
        <section className="bg-slate-900/50 rounded-xl border border-slate-700/30 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm text-slate-400">
              {xpStats.totalIntrinsicsRanks} ranks ({INTRINSIC_XP.toLocaleString()} XP each)
            </div>
          </div>

          {/* Railjack Intrinsics */}
          <div className="mb-6">
            <h4 className="text-md font-semibold text-cyan-400 mb-3">Railjack Intrinsics</h4>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 bg-slate-900 rounded-full h-3 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(rank / MAX_INTRINSIC_RANK) * 100}%` }}
                        />
                      </div>
          ...
          {/* Drifter Intrinsics */}
          <div>
            <h4 className="text-md font-semibold text-purple-400 mb-3">Drifter Intrinsics</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      ...
      {activeTab === "recommendations" && (
        <section className="bg-slate-900/50 rounded-xl border border-yellow-700/30 p-6">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
              {masterableItems
                .filter(item => !masteredItems.has(item.path) && item.xp >= 6000)
                .slice(0, 12)
                .map((item) => {
                  const imageUrl = getItemImageUrl({ uniqueName: item.path });
                  return (
            ...
            {masterableItems.filter(item => !masteredItems.has(item.path) && item.xp >= 6000).length === 0 && (
              <p className="text-center text-green-400 py-4">All high XP items mastered! 🎉</p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
              {masterableItems
                .filter(item => !masteredItems.has(item.path) && item.xp === 3000)
                .slice(0, 20)
                .map((item) => (
                  <button
                    key={item.path}
            ...
            <p className="text-center text-slate-500 text-sm mt-4">
              {masterableItems.filter(item => !masteredItems.has(item.path)).length} items remaining to master
            </p>
          </div>
        </section>
      )}

...
export function NightwaveProgress({
  challenges,
  currentStanding = 0,
}: NightwaveProgressProps) {
  const [completedIds, setCompletedIds] = useLocalStorageSet<string>(
  ...
  const standingStats = useMemo(() => {
    ...
    return {
      ...
      totalCreds:
        NIGHTWAVE_REWARDS.filter(
          (r) => r.rank <= currentRank && r.type === "cred",
        ).length * 50,
    };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-900/30 to-slate-900/50 rounded-xl border border-purple-700/30 p-4">
        ...
        <div className="relative mb-2">
          ...
          <div className="absolute inset-0 flex items-center justify-center text-xs font-medium text-white">
            {standingStats.progressToNextRank.toLocaleString()} /{" "}
            {STANDING_PER_RANK.toLocaleString()}
          </div>
        </div>
      ...
      <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          {NIGHTWAVE_REWARDS.slice(
            standingStats.currentRank,
            standingStats.currentRank + 5,
          ).map((reward) => (
            <div
              key={reward.rank}
    ...
    >
      <div
        className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${
          completed ? "bg-green-600 border-green-600" : "border-slate-500"
        }`}
      >
      ...
      <div className="flex-1 min-w-0">
        <div
          className={`font-medium text-sm ${completed ? "text-slate-400 line-through" : "text-slate-200"}`}
        >
          {challenge.title}
        </div>
      ...
      <div
        className={`text-sm font-bold ${isElite ? "text-yellow-400" : "text-purple-400"}`}
      >

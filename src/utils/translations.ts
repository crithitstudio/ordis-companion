...
export const missionTypes = missionTypesData as Record<string, MissionTypeEntry>;
...
export const getItemCategory = (path: string): string => {
    ...
    if (item) {
        ...
        if (cat === 'Primary' || cat === 'LongGun' || cat === 'Rifle' || cat === 'Shotgun') return 'Primary';
        if (cat === 'Secondary' || cat === 'Pistol') return 'Secondary';
        if (cat === 'Melee') return 'Melee';
        if (cat === 'Mods' || cat === 'Mod') return 'Mod';
        if (cat === 'Resources' || cat === 'Resource' || cat === 'Fish' || cat === 'Gem') return 'Resource';
        return 'Other';
    }
    // Fallback to path-based if item not in itemsData
    if (path.includes('/Warframes/') || path.includes('/Suits/') || path.includes('/Powersuits/')) return 'Warframe';
    ...
    if (path.includes('/Resources/') || path.includes('/Materials/')) return 'Resource';
    return 'Other';
};

            ...
            >
                ...
                <div className="sticky top-0 bg-slate-900 border-b border-slate-700 p-4 flex items-start justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-cyan-400">{itemData.name}</h2>
                            <p className="text-sm text-slate-500">{itemData.type} • {itemData.category}</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {itemData.masteryReq > 0 && (
                            <div className="bg-slate-800/50 rounded-lg p-3">
                                ...
                                <p className="text-lg font-bold text-yellow-400">{itemData.masteryReq}</p>
                            </div>
                        )}
                        {itemData.tradable && (
                            <div className="bg-slate-800/50 rounded-lg p-3">
                        ...
                        {itemData.buildPrice && (
                            <div className="bg-slate-800/50 rounded-lg p-3">
                                ...
                                <p className="text-lg font-bold text-cyan-400">{itemData.buildPrice.toLocaleString()} cr</p>
                            </div>
                        )}
                        {itemData.buildTime && (
                            <div className="bg-slate-800/50 rounded-lg p-3">
                                <p className="text-xs text-slate-500">Build Time</p>
                                <p className="text-lg font-bold text-purple-400">{Math.floor(itemData.buildTime / 3600)}h</p>
                            </div>
                        )}
                    </div>
                    ...
                    {itemData.components && itemData.components.length > 0 && (
                        <div>
                            <h3 className="text-sm font-bold text-slate-400 uppercase mb-2">Crafting Requirements & Costs</h3>
                            <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
                                ...
                                <div className="flex gap-4 mb-4 pb-4 border-b border-slate-700/50">
                                    {itemData.buildPrice && (
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-slate-200 font-bold">{itemData.buildPrice.toLocaleString()}</span>
                                        </div>
                                    )}
                                    {itemData.buildTime && (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    {itemData.components.map((comp, i) => {
                                        ...
                                        if (!compImage) {
                                            const matchedItem = Object.values(itemsData).find((item) => item.name === comp.name);
                                            if (matchedItem) {
                                        ...
                                        return (
                                            <div key={i} className="flex items-center gap-3 p-2 bg-slate-900/40 rounded border border-slate-700/30">
                                                {compImage ? (
                                                    <img src={compImage} alt={comp.name} className="w-8 h-8 object-contain" />
                                                ) : (
                                                    <div className="w-8 h-8 bg-slate-800 rounded flex items-center justify-center text-xs text-slate-500">?</div>
                                                )}
                                                <div className="flex flex-col">
                                                    <span className="text-slate-200 text-sm font-medium">{comp.name}</span>
                                                    <span className="text-cyan-400 text-xs font-mono">x{comp.count}</span>
                                                </div>
                                            </div>
                                        );
                                    })}
                    ...
                    {itemData.drops && itemData.drops.length > 0 && (
                        <div>
                            <h3 className="text-sm font-bold text-slate-400 uppercase mb-2">Drop Locations</h3>
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                {itemData.drops.map((drop, i) => (
                                    <div key={i} className="bg-slate-800/30 rounded-lg p-3 border border-slate-700/50">
                                        <div className="flex items-center justify-between">
                                            <span className="text-slate-300 text-sm">{drop.location}</span>
                                            <span className={`text-xs px-2 py-1 rounded ${drop.rarity === 'Common' ? 'bg-slate-600 text-slate-200' :
                                                drop.rarity === 'Uncommon' ? 'bg-green-700 text-green-100' :
                                                    drop.rarity === 'Rare' ? 'bg-blue-700 text-blue-100' :
                                        ...
                                        {drop.chance > 0 && (
                                            <p className="text-xs text-slate-500 mt-1">{(drop.chance * 100).toFixed(2)}% chance</p>
                                        )}
                                    </div>
                                ))}
                            </div>

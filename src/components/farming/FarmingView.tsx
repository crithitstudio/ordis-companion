...
const RESOURCE_FARMING_LOCATIONS: {
    ...
    missions: { name: string; planet: string; type: string; notes: string }[];
}[] = [
        {
            ...
            missions: [
                { name: "Helene", planet: "Saturn", type: "Defense", notes: "20 waves, General Sargas Ruk drops" },
                { name: "Gabii", planet: "Ceres", type: "Dark Sector Survival", notes: "Stay 20+ minutes" },
            ],
        },
        {
            resource: "Neurodes",
            missions: [
                { name: "Tycho", planet: "Lua", type: "Survival", notes: "Sentients drop guaranteed" },
                { name: "Mariana", planet: "Earth", type: "Exterminate", notes: "Low level, fast runs" },
            ],
        },
        {
            resource: "Argon Crystal",
            missions: [
                { name: "Void Capture", planet: "Void", type: "Capture", notes: "Quick runs, check containers" },
                { name: "Mot", planet: "Void", type: "Survival", notes: "High level, good drops" },
            ],
        },
        {
            resource: "Polymer Bundle",
            missions: [
                { name: "Ophelia", planet: "Uranus", type: "Survival", notes: "Also drops Plastids and Tellurium" },
                { name: "Assur", planet: "Uranus", type: "Dark Sector Survival", notes: "35% bonus resources" },
            ],
        },
        {
            resource: "Plastids",
            missions: [
                { name: "Ophelia", planet: "Uranus", type: "Survival", notes: "Multi-resource farm" },
                { name: "Piscinas", planet: "Saturn", type: "Dark Sector Survival", notes: "20% bonus resources" },
            ],
        },
        {
            resource: "Cryotic",
            missions: [
                { name: "Hieracon", planet: "Pluto", type: "Excavation", notes: "100 per excavator" },
            ],
        },
        {
            resource: "Oxium",
            missions: [
                { name: "Io", planet: "Jupiter", type: "Defense", notes: "Oxium Ospreys spawn often" },
                { name: "Outer Terminus", planet: "Pluto", type: "Defense", notes: "Higher enemy levels" },
            ],
        },
        {
            resource: "Tellurium",
            missions: [
                { name: "Ophelia", planet: "Uranus", type: "Survival", notes: "Best farm location" },
                { name: "Archwing", planet: "Any", type: "Archwing Mission", notes: "Any Archwing mission" },
            ],
        },
    ];
 ...
 */
function findMatchingFissures(eras: Set<string>, fissures: Fissure[]): Fissure[] {
...
export function FarmingView({ fissures }: FarmingViewProps) {
    ...
    const farmingRoutes = useMemo((): FarmingRoute[] => {
        ...
        return Array.from(wantedItems).map((itemName) => {
            const drop = primeDrops.find((p) => p.item === itemName);
            if (!drop) return { item: itemName, relics: [], activeFissures: [], bestFissure: undefined };
    ...
    return (
        <div className="space-y-6">
                                ...
                                {route.bestFissure ? (
                                    <div className="bg-green-900/20 border border-green-700/30 rounded-lg p-3 mb-3">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <p className="text-slate-200">{route.bestFissure.node}</p>
                                                <p className="text-sm text-slate-400">
                                                    {route.bestFissure.tier} • {route.bestFissure.missionType} • {route.bestFissure.enemy}
                                                </p>
                                            </div>
                                    ...
                                    <div className="bg-slate-800/50 border border-slate-700/30 rounded-lg p-3 mb-3">
                                        <p className="text-slate-500 text-sm">
                                            No active fissures for this item's relics. Check back later!
                                        </p>
                                    </div>
                                )}
                                    ...
                                    <details className="text-sm">
                                        ...
                                        <div className="mt-2 space-y-1">
                                            {route.activeFissures.slice(1).map((f) => (
                                                <div key={f.id} className="flex items-center justify-between py-1 px-2 bg-slate-900/50 rounded">
                                                    <span className="text-slate-300">{f.node}</span>
                                                    <span className="text-slate-500">{f.missionType}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </details>
            ...
            <div className="bg-slate-900/50 rounded-xl border border-amber-900/30 p-6">
                ...
                <p className="text-slate-400 mb-4">
                    Search for Prime parts to find which relics contain them and plan your farming.
                </p>
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                title={isWanted ? "Remove from farming list" : "Add to farming list"}
                                            >
                                                        ...
                                                        {matchingFissures.slice(0, 3).map((f) => (
                                                            <div key={f.id} className="flex items-center justify-between text-sm">
                                                                <span className="text-slate-300">
                                                                    <span className={getEraColor(f.tier).split(" ")[0]}>{f.tier}</span> • {f.node}
                                                                </span>
                                                                <span className="text-slate-500">{f.missionType} • {f.eta}</span>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
                                                    ...
                                                    >
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center justify-between mb-1">
                                                                <div className="flex items-center gap-2">
                                                                    <span className={getEraColor(src.era).split(" ")[0]}>
                                                                        {src.relic}
                                                                    </span>
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <span className="text-xs text-slate-400 font-mono w-12 text-right">
                                                                        {src.chance > 0 ? `${src.chance.toFixed(1)}%` : "?%"}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                                ...
                                                                <div className="h-1 bg-slate-700 rounded-full overflow-hidden">
                                                                    <div
                                                                        className={`h-full transition-all ${src.rarity.toLowerCase() === "rare"
                                                                                ? "bg-yellow-500"
                                                                                : src.rarity.toLowerCase() === "uncommon"
                                                                                    ? "bg-blue-500"
                                                                                    : "bg-amber-600"
                                                                            }`}
                                                                        style={{ width: `${Math.min(100, src.chance * 3)}%` }}
                                                                    />
                                                                </div>
                                                            )}
                                                        </div>
                ...
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {fissures.slice(0, 9).map((f) => (
                        ...
                        >
                            ...
                            <p className="text-xs text-slate-500">{f.missionType} • {f.enemy}</p>
                        </div>
                    ))}
                </div>
                {fissures.length > 9 && (
            ...
            <div className="bg-slate-900/50 rounded-xl border border-emerald-900/30 p-6">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {RESOURCE_FARMING_LOCATIONS.map((resource) => (
                        ...
                        >
                            <h3 className="text-emerald-300 font-medium mb-2">{resource.resource}</h3>
                            <div className="space-y-2">
                                {resource.missions.map((mission, idx) => (
                                    <div key={idx} className="text-sm">
                                        <div className="flex items-center justify-between">
                                            <span className="text-slate-200">{mission.name}</span>
                                            <span className="text-xs text-slate-500">{mission.planet}</span>
                                        </div>
                                        <div className="text-xs text-slate-400">{mission.type}</div>
                                        <div className="text-xs text-emerald-500/70 italic">{mission.notes}</div>
                                    </div>
                                ))}
                            </div>
                        </div>

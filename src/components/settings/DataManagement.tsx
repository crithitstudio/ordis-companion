    ...
    reader.onload = (e) => {
      try {
        ...
        if (
          !parsed.version ||
          !parsed.data ||
          parsed.appName !== "Ordis Companion"
        ) {
          setImportError(
            "Invalid backup file format. Please select a valid Ordis Companion backup.",
          );
          return;
        }
        ...
        setImportError(
          "Failed to parse the backup file. Make sure it's a valid JSON file.",
        );
      }
    };
  ...
  const confirmImport = () => {
    ...
    try {
      ...
      setTimeout(() => {
        if (
          confirm(
            "Data imported successfully! Refresh the page now to apply changes?",
          )
        ) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400">
          Export your progress and settings to a backup file, or import from a
          previous backup.
        </p>
      </div>
        ...
        {importing && importPreview && (
          <div className="mt-4 p-4 bg-slate-900/50 border border-yellow-700/50 rounded-lg">
            ...
            <p className="text-sm text-slate-300 mb-3">
              This will <strong>replace</strong> your current data with the
              backup from{" "}
            ...
            <div className="grid grid-cols-2 gap-2 mb-4 text-xs">
              {Object.entries(getPreviewStats() || {})
                .slice(0, 6)
                .map(([key, count]) => (
      ...
      <div className="p-3 bg-yellow-900/20 border border-yellow-700/30 rounded-lg">
        <p className="text-xs text-yellow-300/80">
          <strong>Note:</strong> Importing will overwrite your current data.

...
import { fetchItemPrices, getCachedPrices, type PriceData } from "../../services/priceService";
...
function getRelicData(): Map<string, { drops: string[]; dropRarities: string[] }> {
...
export function RelicPlannerTab() {
    ...
    const [prices, setPrices] = useState<Map<string, PriceData>>(getCachedPrices());
        ...
        return alecaRelics.relics.map(relic => {
            ...
            if (data) {
                data.drops.forEach((item, i) => {
                    ...
                    const chance = rarity === "Rare" ? 0.02 : rarity === "Uncommon" ? 0.11 : 0.2533;
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-purple-900/30 to-slate-900/50 rounded-xl border border-purple-700/30 p-6">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-purple-400">Relic Planner</h2>
                            <p className="text-sm text-slate-400">
                                {alecaRelics.totalRelics.toLocaleString()} relics • Last updated {new Date(alecaRelics.fetchedAt).toLocaleString()}
                            </p>
                        </div>
                    </div>
                    <button
                            ...
                            {topRelics.map((relic, i) => (
                                ...
                                >
                                    ...
                                    <div className="text-purple-400 font-medium">
                                        ~{relic.expectedValue.toFixed(1)} <span className="text-xs">p</span>
                                    </div>
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className="text-center min-w-[60px]">
                                        <div className="text-2xl font-bold text-slate-200">{relic.count}</div>
                            ...
                            {relic.drops.length > 0 && (
                                <div className="mt-3 pt-3 border-t border-slate-700/50 flex flex-wrap gap-2">
                                    {relic.drops.slice(0, 6).map((drop, j) => (
                                        <div
                                            key={j}
                                            className={`px-2 py-1 rounded text-xs ${drop.rarity === "Rare" ? "bg-amber-900/30 text-amber-300" :
                                                drop.rarity === "Uncommon" ? "bg-blue-900/30 text-blue-300" :
                                                    "bg-slate-700/50 text-slate-400"
                                                }`}
                                        >
                                            {drop.item.replace(" Prime", "").replace(" Blueprint", " BP")}
                                            {drop.price > 0 && (
                                                <span className="ml-1 text-purple-400">{drop.price}p</span>
                                            )}
                                        </div>
                                    ))}
                                </div>
            ...
            <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
                ...
                <ul className="text-sm text-slate-400 space-y-1">
                    <li>• Expected value is calculated using drop chances and current market prices</li>
                    <li>• Prices are fetched from <a href="https://warframe.market" target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:text-purple-300">warframe.market</a></li>
                    <li>• Update prices regularly for accurate recommendations</li>
                    <li>• Radiant refinement increases rare drop chance from 2% to 10%</li>
                </ul>
            </div>
        </div>
    );
